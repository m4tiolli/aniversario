...
function Convidados() {
  ...
  const [pesquisa, setPesquisa] = useState<PesquisaItem[]>([]);
  ...
  const onEnter = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === "Enter") {
      ...
      if (addMoreButtonRef.current) {
        addMoreButtonRef.current.click();
      }
    }
  };
  ...
  const enviar = () => {
    names.map((nome) => {
      const body = { nome };
      axios.post(process.env.URL_BASE ?? "", body).catch((err) => console.error(err));
    });
    onModalOpen();
  };
  ...
  const verificarPresenca = () => {
    axios
      .get(`${process.env.URL_BASE ?? ""}/pesquisa/${param}`)
